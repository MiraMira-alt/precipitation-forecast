...
# -- Session state --
for key in ['df','models','best_model','metrics','feature_cols','forecast_results','forecast_df_raw']:
...
with col1:
    ...
    if city_choice == "Інше (координати)":
        ...
        city_name_custom = st.text_input("Назва міста", value="Моє місто")
...
if st.button("Завантажити дані", use_container_width=False):
    ...
    with st.spinner(f"Завантажую дані для {coords['name']} з Open-Meteo..."):
        try:
            ...
            st.markdown(
                f'<div class="success-box">Завантажено <b>{len(df)}</b> записів для <b>{coords["name"]}</b>. '
                f'Збережено у weather_daily.csv</div>',
                unsafe_allow_html=True
            )
    ...
    with st.expander("Переглянути дані", expanded=False):
        ...
        with tab2:
            ...
            fig_pie.update_layout(
                ...
                font=dict(color='white'), showlegend=False, margin=dict(t=20,b=20),
...
if st.session_state.df is None:
    st.markdown('<div class="warn-box">Спочатку завантажте дані у Блоці 1</div>', unsafe_allow_html=True)
else:
    if st.button("Навчити моделі", use_container_width=False):
...
if st.session_state.best_model is None:
    st.markdown('<div class="warn-box">Спочатку навчіть модель у Блоці 2</div>', unsafe_allow_html=True)
else:
    coords = st.session_state.city_coords
    st.markdown(
    ...
    if st.button("Отримати прогноз", use_container_width=False):
    ...
    if st.session_state.forecast_results is not None:
        ...
        forecast_raw = st.session_state.forecast_df_raw
        ...
        if 'temperature_2m_mean' in forecast_raw.columns:
            temps = forecast_raw['temperature_2m_mean'].values[:len(results)]
            fig_fc.add_trace(go.Scatter(
                x=dates, y=temps, name="Температура (°C)",
        ...
        fig_fc.update_yaxes(title_text="Ймовірність (%)", secondary_y=False,
